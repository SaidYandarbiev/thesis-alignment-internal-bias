"""
SFT Script — Sequential: Gender SFT → Religion SFT on Mistral-7B-v0.3 (LoRA)
============================================================================
Implements sequential bias alignment: trains on religion bias data starting
from a model already fine-tuned on gender bias, rather than from the base model.

Step 1: Load the pre-trained gender SFT adapter.
Step 2: Merge the adapter into the base weights with merge_and_unload().
Step 3: Apply a fresh LoRA adapter and train on the religion-extended SFT dataset.
...
OUTPUT_DIR     = "checkpoints/seq_gender_sft_to_religion_sft"
...
# gender alignment is retained or disrupted during religion training.
GENDER_KEYWORDS = [
...
# Race-overlapping examples are dropped to avoid introducing a third bias
...
def is_religion_example(example):
    ...
    single-axis religion bias after the initial gender-SFT alignment.
    """
    text = " ".join([
...
def load_and_preprocess():
    ...
    religion extension for the second stage of sequential SFT training.
...
if __name__ == "__main__":
    ...
    # First-stage adapter: model previously aligned on the gender-extended SFT data.
    # This adapter is merged into the base model before starting religion-SFT training.
    ADAPTER_ID = "../checkpoints/mistral7b_biasdpo_gender_extended_sft"
    ...
    # Merge the gender-SFT LoRA adapter into the model weights so the next LoRA
    # adapter starts from the gender-aligned model rather than from the base model.
    ...
    # alignment is learned on top of the merged gender-aligned model.
    ...
    print("Starting sequential Gender-SFT → Religion-SFT training on Mistral-7B-v0.3 (LoRA, LR=5e-5)...")
    trainer.train()
