"""
DPO Script — Sequential: Race DPO → Gender DPO on Mistral-7B-v0.3 (LoRA)
===========================================================================
Implements sequential bias alignment: trains on gender bias data starting
from a model already fine-tuned on race bias (not from the base model).

Step 1: Load the pre-trained race DPO adapter (mistral7b_biasdpo_race_extended_dpo)
Step 2: Merge the adapter into the base weights (merge_and_unload)
Step 3: Apply a fresh LoRA adapter and train on the gender-extended dataset
...
OUTPUT_DIR     = "checkpoints/seq_race_dpo_to_gender_dpo"
...
def is_gender_example(example):
    ...
    single-axis gender bias after the initial race-DPO alignment.
    """
    text = " ".join([
...
def load_and_preprocess():
    ...
    gender extension for the second stage of sequential DPO training.
...
if __name__ == "__main__":
    ...
    # This adapter is merged into the base model before starting gender-DPO training.
    ...
    # A fresh LoRA adapter is trained for the second stage so that gender alignment
    # is learned on top of the merged race-aligned model.
    ...
    print("Starting sequential Race-DPO → Gender-DPO training on Mistral-7B-v0.3 (LoRA, LR=5e-5)...")
    trainer.train()
