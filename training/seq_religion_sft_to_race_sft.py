"""
SFT Script — Sequential: Religion SFT → Race SFT on Mistral-7B-v0.3 (LoRA)
==========================================================================
...
Step 1: Load the pre-trained religion SFT adapter.
...
OUTPUT_DIR     = "checkpoints/seq_religion_sft_to_race_sft"
...
def is_race_example(example):
    ...
    single-axis race bias after the initial religion-SFT alignment.
    """
    text = " ".join([
...
if __name__ == "__main__":
    ...
    # First-stage adapter: model previously aligned on the religion-extended SFT data.
    # This adapter is merged into the base model before starting race-SFT training.
    ADAPTER_ID = "../checkpoints/mistral7b_biasdpo_religion_extended_sft"
    ...
    # Merge the religion-SFT LoRA adapter into the model weights so the next LoRA
    ...
    # is learned on top of the merged religion-aligned model.
    ...
    print("Starting sequential Religion-SFT → Race-SFT training on Mistral-7B-v0.3 (LoRA, LR=5e-5)...")
    trainer.train()
