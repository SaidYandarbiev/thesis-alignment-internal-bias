"""
SFT Script — Sequential: Gender SFT → Race SFT on Mistral-7B-v0.3 (LoRA)
=========================================================================
...
Step 3: Apply a fresh LoRA adapter and train on the race-extended SFT dataset.
...
OUTPUT_DIR     = "checkpoints/seq_gender_sft_to_race_sft"
...
# gender alignment is retained or disrupted during race training.
GENDER_KEYWORDS = [
...
def is_race_example(example):
    ...
    single-axis race bias after the initial gender-SFT alignment.
    """
    text = " ".join([
...
def load_and_preprocess():
    ...
    race extension for the second stage of sequential SFT training.
...
if __name__ == "__main__":
    ...
    # This adapter is merged into the base model before starting race-SFT training.
    ...
    # A fresh LoRA adapter is trained for the second stage so that race alignment
    # is learned on top of the merged gender-aligned model.
    ...
    print("Starting sequential Gender-SFT → Race-SFT training on Mistral-7B-v0.3 (LoRA, LR=5e-5)...")
    trainer.train()
