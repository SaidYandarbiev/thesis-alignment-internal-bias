"""
DPO Script — Sequential: Gender DPO → Religion DPO on Mistral-7B-v0.3 (LoRA)
===============================================================================
Implements sequential bias alignment: trains on religion bias data starting
from a model already fine-tuned on gender bias (not from the base model).

Step 1: Load the pre-trained gender DPO adapter (mistral7b_biasdpo_gender_extended_dpo)
Step 2: Merge the adapter into the base weights (merge_and_unload)
...
OUTPUT_DIR     = "checkpoints/seq_gender_dpo_to_religion_dpo"
...
def is_religion_example(example):
    ...
    single-axis religion bias after the initial gender-DPO alignment.
    """
    text = " ".join([
...
if __name__ == "__main__":
    ...
    # First-stage adapter: model previously aligned on the gender-extended DPO data.
    # This adapter is merged into the base model before starting religion-DPO training.
    ADAPTER_ID = "../checkpoints/mistral7b_biasdpo_gender_extended_dpo"
    ...
    # Merge the gender-DPO LoRA adapter into the model weights so the next LoRA
    ...
    print("Starting sequential Gender-DPO → Religion-DPO training on Mistral-7B-v0.3 (LoRA, LR=5e-5)...")
    trainer.train()
